...
public class P2CboCheckBoxListObject<E> extends MenuButton {
    private final StringProperty filterProperty = new SimpleStringProperty();
    private final ObservableList<E> showList;
    private final ObservableList<E> selList;
    private final ArrayList<MenuItemClass> menuItemsList = new ArrayList<>();

    public P2CboCheckBoxListObject(ObservableList<E> showList,
                                   ObservableList<E> selList) {
...
//            updateMenuButton();
//        });
        showList.addListener((ListChangeListener<E>) c -> updateMenuButton());
        selList.addListener((ListChangeListener<E>) c -> updateMenuButton());
//        textProperty().bindBidirectional(filterProperty);
    ...
    private void updateMenuButton() {
        ...
        for (E s : showList) {
            ...
            MenuItemClass menuItemClass = new MenuItemClass(s.toString(), miCheck);
            menuItemsList.add(menuItemClass);

            Button btnChannel = new Button(s.toString());
